...
# Example:     
# ---------------------------------------------------------------------
# Author:       Zografia Anastasiadou 20111102
# ---------------------------------------------------------------------
...
c2<-numeric(1:d^2)
for (i in 1:d^2){
	c2[i]=((1+y[i,1]^2/v)*(1+y[i,2]^2/v))^((v+1)/2)/(1+(y[i,]%*%solve(R)%*%y[i,])/v)^(v/2+1)
	}
...
wireframe(f~u*w,s,shade=TRUE,xlab="X",ylab="Y",zlab="Z",main="t-Student Copula Density, \\nu = 3, r = 0.4",scales=list(arrows=FALSE))